...
function buildNavigation(info) {
  ...
  if (info.prefix) {
    ...
    //$('#navigation2').html(root + content.join(' / '));
  } else {
    //$('#navigation2').html(root);
  }
}
    ...
    if (item.Type === 'directory') {
      if (S3BL_IGNORE_PATH) {
        item.href = location.protocol + '//' + location.hostname + ':' +
                    location.port + location.pathname + '?prefix=' + item.Key;
      } else {
        item.href = item.keyText;
      }
    } else {
      item.href = 'https://www.eossnapshots.io/' + encodeURIComponent(item.Key);
